    ...
    return (
        <div className='grid sm:grid-cols-1 lg:grid-cols-3'>
            <div className='p-4 mb-8 sm:order-10'>
                 ...
                 {
                    datas.map(course => <li key={course.id} className='mt-5 '><Link to={`/courses/${course.id}`} className='text-[#FEC32D] py-1 bg-gray-900 px-2 underline rounded'>{course.title}</Link></li>)
                 }
            </div>
